...
- `BusinessLogicException` - исключение для ошибок бизнес-логики приложения. Предназначено для информирования
  ...
  клиенту дополнительной информации об ошибке. Для указания сообщения данное исключение позволяет указать `ErrorCode`.
  Через него автоматически будет указан как код ошибки, так и читаемое сообщение. Это даст больше возможностей клиенту
  обработать данную ошибку.
...
Для указания сообщения предназначен обязательный параметр с типом `ErrorCode`. Через него указывается и код сообщения, и
сообщение. Так как исключение бизнес-логики подразумевает обработку его на клиенте, указание всегда кода ошибки даст
клиенту больше возможностей для обработки данного исключения.
